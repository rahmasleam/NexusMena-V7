...
const ArticleDetail: React.FC = () => {
    ...
    // Auto-generate summary when item is loaded
    useEffect(() => {
        if (item && !aiContent) {
    ...
    const handleGenerateInsight = async () => {
        ...
        try {
            ...
            setAiContent("Could not generate insight at this time.");
        }
    };
    ...
    const handleToggleReaderMode = async () => {
        ...
        if (!articleText && item) {
            ...
            // Better search context: Title + Source
            const searchContext = `${item.title} ${item.source || item.name || ''}`;
            const content = await getArticleContent(item.url || item.website || item.registrationLink, searchContext);
